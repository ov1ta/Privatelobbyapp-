...
export default function Home() {
  ...
  const shimmerHeights = useRef(Array(8).fill(0).map(() => 140 + Math.floor(Math.random() * 120)))
      ...
      <div style={{ maxWidth: '1600px', margin: '0 auto' }}>
        ...
        {loading && (
          <div className="masonry-grid" style={{ padding: '20px 24px' }}>
            {shimmerHeights.current.map((h, i) => (
              <div key={i} className="masonry-item">
                <div className="shimmer" style={{ height: `${h}px`, borderRadius: '16px' }} />
              </div>
            ))}
          </div>
        )}
