  ...
  async function submit(e) {
    ...
    try {
      if (mode === 'login') await signIn(form.email, form.password)
        ...
        await signUp(form.email, form.password, form.username, form.inviteCode)
        setEmailSent(true)
      }
    } catch (err) {
        ...
        <div className="card" style={{ padding: '32px' }}>
          {emailSent ? (
            <div style={{ textAlign: 'center', padding: '20px 0' }}>
              ...
              <h2 style={{ fontSize: '20px', fontWeight: '800', marginBottom: '10px' }}>Check your email</h2>
              <p style={{ color: 'var(--text-muted)', fontSize: '14px', lineHeight: 1.6 }}>
                We sent a confirmation link to <strong style={{ color: 'var(--text-primary)' }}>{form.email}</strong>.<br />
                Click it to activate your account, then sign in.
              </p>
              <button onClick={() => { setEmailSent(false); setMode('login') }} className="btn btn-primary" style={{ marginTop: '24px', width: '100%', justifyContent: 'center', padding: '13px' }}>
                Back to Sign In
              </button>
            </div>
          ) : (
